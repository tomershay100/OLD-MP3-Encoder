...
class MP3Encoder:
    ...
    def encode(self):
        ...
        subband_samples = np.zeros((self.__wav_file.num_of_ch, tables.N_SUBBANDS, tables.FRAMES_PER_BLOCK),
                                   dtype='float32')

        # Main loop, executing until all samples have been processed.
        while self.__wav_file.num_of_processed_samples < self.__wav_file.num_of_samples:
            ...
            for frm in range(tables.FRAMES_PER_BLOCK):
                ...
                if samples_read < tables.SHIFT_SIZE:
                    for ch in range(self.__wav_file.num_of_ch):
            ...
            scfindices = np.zeros((self.__wav_file.num_of_ch, tables.N_SUBBANDS), dtype='uint8')
            subband_bit_allocation = np.zeros((self.__wav_file.num_of_ch, tables.N_SUBBANDS), dtype='uint8')
            ...
            for ch in range(self.__wav_file.num_of_ch):
                scfindices[ch, :] = util.get_scale_factors(subband_samples[ch, :, :],
                                                           self.__wav_file.table.scale_factor)
            ...
            for ch in range(self.__wav_file.num_of_ch):
                for sb in range(tables.N_SUBBANDS):
                    QCa = self.__wav_file.table.qca[subband_bit_allocation[ch, sb] - 2]
                    QCb = self.__wav_file.table.qcb[subband_bit_allocation[ch, sb] - 2]
                    scf = self.__wav_file.table.scale_factor[scfindices[ch, sb]]
